...
public:
	...
	void init_texture_map()
	{
		...
		int TexTop = MyLoadCubeMapBitmap("Skybox\\top.bmp", GL_TEXTURE_CUBE_MAP_POSITIVE_Y);
		int TexBottom = MyLoadCubeMapBitmap("Skybox\\bottom.bmp", GL_TEXTURE_CUBE_MAP_NEGATIVE_Y);
	...
	{		
		start_shader();
	        ...
	        glUniformMatrix4fv(uniforms[0], 1, GL_TRUE, modelview * Scale(100.0, 100.0, 100.0) * RotateZ(theta[2]) * RotateX(theta[0]) * RotateY(theta[1]) );
			GL_CHECK_ERRORS
