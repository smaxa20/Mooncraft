...
class Block : public graphics_object
{
private:
	vec3 position;
	char* textureFile;
	mat4 scale;
public:
	Block(char* textureFile, mat4 scale) : graphics_object("SkyCube") 
	{ 
	...
	{	 // For this texture map, the normal are set to
		 // normalized versions of the vertex itself
	...
	void init_texture_map()
	{
		...
		int TexRight = MyLoadCubeMapBitmap(textureFile, GL_TEXTURE_CUBE_MAP_POSITIVE_X);
		int TexLeft = MyLoadCubeMapBitmap(textureFile, GL_TEXTURE_CUBE_MAP_NEGATIVE_X);
		int TexTop = MyLoadCubeMapBitmap(textureFile, GL_TEXTURE_CUBE_MAP_POSITIVE_Y);
		int TexBottom = MyLoadCubeMapBitmap(textureFile, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y);
		int TexFront = MyLoadCubeMapBitmap(textureFile, GL_TEXTURE_CUBE_MAP_POSITIVE_Z);
		int TexBack = MyLoadCubeMapBitmap(textureFile, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z);
	...
	void draw ( GLfloat theta[], vec3 translate, bool fill ) 
	{
		position = translate;
		start_shader();
			
			// ship down the new the projection and viewing matrices
	        glUniformMatrix4fv(uniforms[2], 1, GL_TRUE, modelview * Translate(translate) * scale * RotateZ(theta[0]) * RotateX(theta[1]) * RotateY(theta[1]) );
			GL_CHECK_ERRORS
